...
function extractCurrencyAndAmount(text: string): { amount?: number; currency?: 'USD' | 'CHF' } {
    const patterns = [
        // Onlydust
    ...
    for (const pattern of patterns) {
        ...
        if (match) {
            try {
                ...
                const amount = parseFloat((isCurrencyFirst ? match[2] : match[1]).replace(/[,\s]/g, ''));
                const currency = (isCurrencyFirst ? match[1] : match[2]).toUpperCase() as 'USD' | 'CHF';
...
function extractExchangeRate(text: string): number {
    ...
    if (usdEurMatch?.[1]) return parseFloat((1 / parseFloat(usdEurMatch[1])).toFixed(4));
    return 0;
}
...
export async function extractInvoiceData(file: File): Promise<ExtractedInvoiceData> {
    try {
        ...
        const { amount: afterTaxAmount, currency } = extractCurrencyAndAmount(extractedText);
