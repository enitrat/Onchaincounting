        ...
        for (let month = 0; month < 12; month++) {
            ...
            if (monthInvoices.length > 0) {
                ...
                groups.push({
                    ...
                    // Native currency totals
                    totalBeforeTaxUsdAmount: usdInvoices.reduce((sum, inv) => sum + inv.beforeTaxAmount, 0),
                    totalAfterTaxUsdAmount: usdInvoices.reduce((sum, inv) => sum + inv.afterTaxAmount, 0),
                    totalBeforeTaxChfAmount: chfInvoices.reduce((sum, inv) => sum + inv.beforeTaxAmount, 0),
                    totalAfterTaxChfAmount: chfInvoices.reduce((sum, inv) => sum + inv.afterTaxAmount, 0),
                    // EUR totals
                    totalBeforeTaxEurAmount: monthInvoices.reduce((sum, inv) => sum + inv.beforeTaxEurAmount, 0),
                    totalAfterTaxEurAmount: monthInvoices.reduce((sum, inv) => sum + inv.afterTaxEurAmount, 0),
                    totalVatEurAmount: monthInvoices.reduce((sum, inv) => sum + inv.vatEurAmount, 0),
                });
            }
        }
                ...
                {monthlyGroups.map((group) => (
                    <Card key={group.month}>
                        <CardHeader>
                            <div className="flex justify-between items-center">
                                <div>
                                    <CardTitle>{monthNames[group.month]} {group.year}</CardTitle>
                                    <CardDescription>Monthly invoice summary</CardDescription>
                                </div>
                                <div className="text-sm text-muted-foreground">
                                    <div>Total EUR (converted): €{group.totalBeforeTaxEurAmount.toFixed(2)}</div>
                                    <div>Total VAT EUR: €{group.totalVatEurAmount.toFixed(2)}</div>
                                    <div>Total After Tax EUR: €{group.totalAfterTaxEurAmount.toFixed(2)}</div>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead className="text-right">After Tax Amount</TableHead>
