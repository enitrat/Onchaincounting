...
export function Dashboard() {
    ...
    const [activeTab, setActiveTab] = React.useState<'overview' | 'invoices' | 'withdrawals'>('overview')
    ...
    const yearData = useLiveQuery(async () => {
        ...
        const summary: YearSummary = {
            year: selectedYear,
            invoices: {
                totalBeforeTaxUsdAmount: invoices.filter(inv => inv.currency === 'USD')
                    .reduce((sum, inv) => sum + inv.beforeTaxAmount, 0),
                totalAfterTaxUsdAmount: invoices.filter(inv => inv.currency === 'USD')
                    .reduce((sum, inv) => sum + inv.afterTaxAmount, 0),
                totalBeforeTaxChfAmount: invoices.filter(inv => inv.currency === 'CHF')
                    .reduce((sum, inv) => sum + inv.beforeTaxAmount, 0),
                totalAfterTaxChfAmount: invoices.filter(inv => inv.currency === 'CHF')
                    .reduce((sum, inv) => sum + inv.afterTaxAmount, 0),
                totalBeforeTaxEurAmount: invoices.reduce((sum, inv) => sum + inv.beforeTaxEurAmount, 0),
                totalAfterTaxEurAmount: invoices.reduce((sum, inv) => sum + inv.afterTaxEurAmount, 0),
                totalVatEurAmount: invoices.reduce((sum, inv) => sum + inv.vatEurAmount, 0),
                count: invoices.length,
            },
            withdrawals: {
                totalSourceUSD: withdrawals.reduce((sum, w) => sum + (w.sourceCurrency === 'USDC' ? w.sourceAmount : 0), 0),
                totalTargetEUR: withdrawals.reduce((sum, w) => sum + w.targetAmount, 0),
                count: withdrawals.length,
            },
        }

        const monthSummaries: MonthSummary[] = Array.from({ length: 12 }, (_, month) => {
            const monthInvoices = invoices.filter(inv => new Date(inv.date).getMonth() === month)
            const monthWithdrawals = withdrawals.filter(w => new Date(w.date).getMonth() === month)

            const usdInvoices = monthInvoices.filter(inv => inv.currency === 'USD')
            const chfInvoices = monthInvoices.filter(inv => inv.currency === 'CHF')

            const monthData = {
                month,
                invoicesUSD: usdInvoices.reduce((sum, inv) => sum + inv.beforeTaxAmount, 0),
                invoicesCHF: chfInvoices.reduce((sum, inv) => sum + inv.beforeTaxAmount, 0),
                invoicesEUR: monthInvoices.reduce((sum, inv) => sum + inv.beforeTaxEurAmount, 0),
                withdrawalsEUR: monthWithdrawals.reduce((sum, w) => sum + w.targetAmount, 0),
                profitEUR: monthInvoices.reduce((sum, inv) => sum + inv.beforeTaxEurAmount, 0) -
        ...
        let runningUSD = 0, runningCHF = 0, runningEUR = 0
            ...
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as typeof activeTab)} className="space-y-8">
                            ...
                            >
                                <ResponsiveContainer width="100%" height="100%">
                                    <ComposedChart data={monthSummaries}>
                                        ...
                                        <XAxis dataKey="month" tickFormatter={(value) => monthNames[value].substring(0, 3)} />
                                        <YAxis yAxisId="left" orientation="left" />
                                        <YAxis yAxisId="right" orientation="right" />
                                        <ChartTooltip content={<ChartTooltipContent />} />
                                        <Legend />
                                        <Bar yAxisId="left" dataKey="invoicesEUR" name="Monthly Income (EUR)" fill="var(--color-monthlyEUR)" />
                                        <Line
                                            yAxisId="right"
                                            type="monotone"
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead className="text-right">Withdrawals (EUR)</TableHead>
                                    ...
                                    {monthSummaries.map((month) => (
                                        <TableRow key={month.month}>
                                            ...
                                            <TableCell className="text-right">
                                                ${month.invoicesUSD.toFixed(2)} / CHF {month.invoicesCHF.toFixed(2)}
                                                <br />
                                                <span className="text-sm text-muted-foreground">
                                                    €{month.invoicesEUR.toFixed(2)}
                                                </span>
                                            </TableCell>
                                            <TableCell className="text-right">€{month.withdrawalsEUR.toFixed(2)}</TableCell>
                                            <TableCell className={`text-right font-medium ${
                                                month.profitEUR > 0 ? 'text-green-600' : 'text-red-600'
                                            }`}>
                ...
                <TabsContent value="invoices">
                    <Card>
                        ...
                        <CardContent>
                            <p className="text-muted-foreground">Invoice details will be displayed here.</p>
                        </CardContent>
                    </Card>
                </TabsContent>
                <TabsContent value="withdrawals">
                    <Card>
                        ...
                        <CardContent>
                            <p className="text-muted-foreground">Withdrawal details will be displayed here.</p>
                        </CardContent>
                    </Card>
                </TabsContent>
            </Tabs>
