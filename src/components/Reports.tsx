...
export function Reports() {
    ...
    const [selectedReport, setSelectedReport] = React.useState<'tax' | 'profit-loss' | 'vat'>('tax');
    ...
    const report = useLiveQuery(async () => {
        ...
        const vatDeductible = expenses
            ...
            .reduce((sum, exp) => sum + (exp.amount * 0.20), 0); // Assuming 20% VAT rate

        // Calculate withdrawal profit/loss
        const totalSourceUSD = withdrawals.reduce((sum, w) =>
            sum + (w.sourceCurrency === 'USDC' ? w.sourceAmount : 0), 0);
        const totalTargetEUR = withdrawals.reduce((sum, w) => sum + w.targetAmount, 0);
        ...
        return {
            ...
            summary: {
                netIncomeUSD: invoices.reduce((sum, inv) => sum + inv.afterTaxAmount, 0) -
                    expenses.reduce((sum, exp) =>
                        sum + (exp.currency === 'USD' ? exp.amount : 0), 0),
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-lg shadow p-6">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">${report.income.totalUSD.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                            <span>VAT Collected</span>
                            <span className="font-medium">${report.income.totalVAT.toFixed(2)}</span>
                        </div>
                ...
                <div className="bg-white rounded-lg shadow p-6">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">${report.expenses.totalUSD.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                            <span>Total Expenses (EUR)</span>
                            <span className="font-medium">€{report.expenses.totalEUR.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                            <span>VAT Deductible</span>
                            <span className="font-medium">${report.expenses.vatDeductible.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
                ...
                {/* Withdrawals Section */}
                <div className="bg-white rounded-lg shadow p-6">
                    <h2 className="text-lg font-semibold text-gray-700 mb-4">Withdrawals</h2>
                    <div className="space-y-2">
                        <div className="flex justify-between">
                            <span>Total Withdrawn (USD)</span>
                            <span className="font-medium">${report.withdrawals.totalSourceUSD.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                            <span>Total Received (EUR)</span>
                            <span className="font-medium">€{report.withdrawals.totalTargetEUR.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className={`font-medium ${
                                report.withdrawals.profitLoss >= 0 ? 'text-green-600' : 'text-red-600'
                            }`}>
                ...
                <div className="bg-white rounded-lg shadow p-6">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between">
                            ...
                            <span className={`font-medium ${
                                report.summary.netIncomeUSD >= 0 ? 'text-green-600' : 'text-red-600'
                            }`}>
                        ...
                        <div className="flex justify-between">
                            <span>Exchange Profit/Loss</span>
                            <span className={`font-medium ${
                                report.summary.netProfitLoss >= 0 ? 'text-green-600' : 'text-red-600'
                            }`}>
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">${report.summary.vatPayable.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
            </div>
